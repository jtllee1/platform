const movePaddle = () => {
  ...
  function movementLoop() {
    ...
    let right = document.getElementById(parseInt(actives[4].id) + 1);
    let left = document.getElementById(parseInt(actives[0].id) - 1);

    if (keyState[39] && right) {
      document.getElementById(parseInt(actives[4].id) + 1).classList.add("active");
      ...
      actives[4].classList.remove("active");
      if (gameState.innerText == "Off") {
        document.getElementById(parseInt(ball.id) - 1).classList.add("ball");
        ball.classList.remove("ball");
      };
